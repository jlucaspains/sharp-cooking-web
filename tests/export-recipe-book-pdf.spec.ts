...
test.describe('Export Recipe Book - PDF Generation', () => {
  ...
  test('should generate PDF with table of contents', async ({ page }) => {
    ...
    // Verify success message appears (indicates PDF was generated)
    const successMessage = page.locator('text=Recipe book downloaded successfully!');
    await expect(successMessage).toBeVisible();
  });
  ...
  test('should NOT include nutrition facts in PDF', async ({ page }) => {
    await setupTestRecipes(page, 1);
    ...
    // Export and verify success (the service excludes nutrition by design)
    await page.click('button:has-text("Export PDF")');
    await page.waitForTimeout(2000);
  ...
  test('should NOT include recipe notes in PDF', async ({ page }) => {
    await setupTestRecipes(page, 1);
    ...
    // Export and verify success (the service excludes notes by design)
    await page.click('button:has-text("Export PDF")');
    await page.waitForTimeout(2000);
